(() => {
  ...
  function render(p) {
    ...
    if (one) {
      // Full-bleed when parked; triangle mainW while the gesture is live
      targetH = layouts.full.mainH;
      targetW = phoneLive ? layouts.peek.mainW : layouts.full.mainW;
    ...
    setters.forEach((set, i) => {
      ...
      if (one) {
        // Visible triangle choreography mid-swipe (same idea as desktop)
        const parkFrom = layouts.full[slotName(i, k)];
        const parkTo = layouts.full[slotName(i, k2)];
        const parkSlot =
          morphing || forceMorph ? lerpSlot(parkFrom, parkTo, t) : parkFrom;
        ...
        const triSlot =
          morphing || forceMorph ? lerpSlot(triFrom, triTo, t) : triFrom;
        const pull = phoneLive ? Math.max(gestS, 0.22) : gestS;
        slot = phoneLive ? lerpSlot(parkSlot, triSlot, pull) : parkSlot;
        set.panel.classList.remove("is-phone-front", "is-phone-peek");
      }

(() => {
  ...
  function layout() {
    ...
    // Narrow: parked = full-bleed main; mid-swipe = visible desktop-like triangle
    if (oneCard) {
      const mainW = stageW * 0.78;
      const mainH = stageH;
      const mainPeekOverlap = 36;
      const peekStackOverlap = Math.round(stageH * 0.2);
      const peekLeft = padX + mainW - mainPeekOverlap;
      const peekW = Math.max(96, vw - padX - peekLeft);
      const peekBand = (stageH - gap) / 2;
      const nextS = Math.min(0.5, Math.max(0.36, (peekW / mainW) * 1.14));
      ...
      // Parked: true full-viewport main (triangle mid-swipe uses `tri` above)
      const parkBottom = 28; // room for dots
      const parkH = Math.max(200, vh - headerBottom - parkBottom);
      const park = {
        mainW: vw,
        mainH: parkH,
        main: { x: 0, y: headerBottom, s: 1, o: 1, r: 0, z: 5 },
        next: {
          x: vw + 28,
          y: nextTop,
          s: nextS,
  ...
  function render(p) {
    ...
    if (one) {
      targetW = phoneSettled ? layouts.full.mainW : layouts.peek.mainW;
      targetH = phoneSettled ? layouts.full.mainH : layouts.peek.mainH;
    ...
    setters.forEach((set, i) => {
      // Phone settled: always write full vw (beat stale peek _w from sizeCards)
      if (one && phoneSettled) {

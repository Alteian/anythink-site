(() => {
  ...
  /** Phone only: one visible full card; every other scene off-screen + o:0 */
  ...
  function endTouch() {
    ...
    let target = Math.round(p0);
    // Prefer nearest page from current + velocity projection (no hard cut)
    if (wasLocked) {
      const projected = viewP + vel * 0.18;
      if (delta > CFG.touchCommit || vel > 0.7 || projected > p0 + 0.45)
        target = Math.floor(p0) + 1;
      else if (delta < -CFG.touchCommit || vel < -0.7 || projected < p0 - 0.45)

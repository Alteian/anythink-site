(() => {
  ...
  function layout() {
    ...
    // Narrow: mini triangle — peeks tucked right; at rest we park full-bleed
    // (peeks only show mid-swipe via render + deck-morphing).
    if (oneCard) {
      ...
      const peekW = Math.max(88, vw - padX - peekLeft);
      ...
      function placePhone(cellLeft, cellTop, cellW, cellH, s, o, z) {
      ...
      const tri = {
        ...
        mainH,
        main: { x: padX, y: padY, s: 1, o: 1, r: 0, z: 5 },
        next: placePhone(nextLeft, nextTop, peekW, peekBand, nextS, 0.95, 4),
        further: placePhone(
          furtherLeft,
          furtherTop,
          peekW,
      ...
      // Parked: full-bleed main; peeks wait off the right so a swipe
      // pulls them in from the side along the triangle path.
      const park = {
        mainW: stageW,
        mainH: stageH,
        main: { x: padX, y: padY, s: 1, o: 1, r: 0, z: 5 },
  ...
  function render(p) {
    ...
    // Desktop + phone page slots (phone blends park↔triangle below)
    const layFrom = layForPage(k);
    const layTo = layForPage(k2);

    // Phone width: freeze for the whole gesture (transforms only mid-drag).
    // Desktop still lerps mainW across peek→full.
    let targetW;
    ...
    // Phone pull amount: 0 at rest → 1 once you’ve left the integer a little
    const away = Math.min(Math.abs(clamped - Math.round(clamped)), 0.5);
    const gest = one ? Math.min(1, away / 0.11) : 1;
    const gestS = gest * gest * (3 - 2 * gest); // smoothstep
    ...
    setters.forEach((set, i) => {
      ...
      if (one) {
        // Continuous: park (side peeks hidden) ↔ triangle path with drag/snap
        ...
        slot = lerpSlot(parkSlot, triSlot, phoneLive ? Math.max(gestS, 0.2) : gestS);
        if (!phoneLive) slot = parkSlot;
      }

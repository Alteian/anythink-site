(() => {
  ...
  function layout() {
    ...
    // Narrow: readable full-bleed front; triangle peeks are background only
    // (scaled previews behind — never shrink the reading card).
    if (oneCard) {
      const mainW = stageW;
      const mainH = stageH;
      const peekStackOverlap = Math.round(stageH * 0.2);
      const peekBand = (stageH - gap) / 2;
      const peekLeft = padX + Math.round(stageW * 0.62);
      const peekW = Math.max(96, vw - padX - peekLeft);
      const nextS = Math.min(0.46, Math.max(0.34, (peekW / mainW) * 1.05));
      const furtherS = nextS * 0.9;
      const nextTop = padY + peekBand + gap - peekStackOverlap;
      const furtherTop = padY;

      function placePhone(cellLeft, cellTop, s, o, z) {
        const fittedW = mainW * s;
        return {
          x: cellLeft + Math.max(0, peekW - fittedW),
          y: cellTop,
          s,
          o,
          r: 0,
      ...
      const tri = {
        ...
        mainH,
        main: { x: padX, y: padY, s: 1, o: 1, r: 0, z: 5 },
        next: placePhone(peekLeft, nextTop, nextS, 0.9, 4),
        further: placePhone(peekLeft, furtherTop, furtherS, 0.75, 3),
        enter: {
          x: vw + 72,
          y: furtherTop,
          s: furtherS,
  ...
  function render(p) {
    ...
    const phoneLive = one && (phoneDragging || !!snapTween || morphing);
    ...
    if (one) {
      // Always full-bleed metrics on phone — never shrink reading cards
    ...
    setters.forEach((set, i) => {
      ...
      let slot;
      let z = stackZ(i, clamped);

      if (one) {
        const park = layouts.full;
        const tri = layouts.peek;
        const isFront = i === k || i === k2;
        if (isFront) {
          // Readable layer: full screen, no scale — crossfade between pages
          const o = !morphing && !forceMorph
            ? i === k
              ? 1
              : 0
            : i === k
              ? 1 - t
              : t;
          const shift = phoneLive ? gestS * -18 : 0;
          slot = {
            x: park.main.x + shift,
            y: park.main.y,
            s: 1,
          ...
          z = 50 + (i === k2 ? 1 : 0);
          set.panel.classList.toggle("is-phone-front", o > 0.05);
          set.panel.classList.toggle("is-phone-peek", false);
        } else {
          // Background triangle only (further cards) — scale OK here
          const from = (morphing || forceMorph ? tri : park)[slotName(i, k)];
          const to = (morphing || forceMorph ? tri : park)[slotName(i, k2)];
          const triSlot =
            morphing || forceMorph ? lerpSlot(from, to, t) : from;
          const parkFrom = park[slotName(i, k)];
          const parkTo = park[slotName(i, k2)];
          const parkSlot =
            morphing || forceMorph
              ? lerpSlot(parkFrom, parkTo, t)
              : parkFrom;
          const pull = phoneLive ? Math.max(gestS, 0.15) : gestS;
          slot = lerpSlot(parkSlot, triSlot, pull);
          if (!phoneLive) slot = { ...parkSlot, o: 0 };
          z = 10 + Math.max(0, 3 - Math.abs(i - k));
          set.panel.classList.toggle("is-phone-front", false);
          set.panel.classList.toggle("is-phone-peek", slot.o > 0.05);
        }
      } else {
      ...
      applySlot(set, slot, z);

      const pe = one
        ? slot.o > 0.55 && set.panel.classList.contains("is-phone-front")
          ? "auto"
          : "none"
        : slot.o > 0.35
          ? "auto"

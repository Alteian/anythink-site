(() => {
  ...
  function layout() {
    ...
    // Narrow: main card ALWAYS full vw; peeks do triangle over the right edge
    if (oneCard) {
      const parkBottom = 28;
      const parkH = Math.max(200, vh - headerBottom - parkBottom);
      const mainW = vw;
      const mainH = parkH;
      const peekStackOverlap = Math.round(parkH * 0.2);
      const peekBand = (parkH - gap) / 2;
      // Sit peeks on the right, tucked over the full-width main
      const peekLeft = Math.round(vw * 0.58);
      const peekW = Math.max(96, vw - peekLeft - 6);
      const nextS = Math.min(0.48, Math.max(0.34, (peekW / mainW) * 1.12));
      const furtherS = nextS * 0.92;
      const nextTop = headerBottom + peekBand + gap - peekStackOverlap;
      const furtherTop = headerBottom;

      function placePhone(cellLeft, cellTop, cellW, s, o, z) {
      ...
      const tri = {
        ...
        mainH,
        main: { ...mainSlot },
        next: placePhone(peekLeft, nextTop, peekW, nextS, 0.95, 4),
        further: placePhone(peekLeft, furtherTop, peekW, furtherS, 0.88, 3),
        enter: {
          x: vw + 72,
          y: furtherTop,
          s: furtherS,
  ...
  function render(p) {
    ...
    // NOTE: `morphing` (k!==k2) is true on every non-last integer page because
    // k2 is always k+1. Settled must key off fractional t / snap / finger.
    const atRest = Math.abs(clamped - Math.round(clamped)) < 1e-4;
    const phoneSettled = one && !phoneDragging && !snapTween && atRest;
    const phoneLive = one && !phoneSettled;
    ...
    if (one) {
      // Main card never shrinks on phone — full vw parked and mid-swipe
    ...
    const gest = one ? Math.min(1, away / 0.11) : 1;
    const gestS = gest * gest * (3 - 2 * gest);

    setters.forEach((set, i) => {
      // Phone: always full vw (parked and mid-swipe)
      if (one) {
        gsap.set(set.panel, { width: targetW, height: targetH });
        set._w = targetW;
      } else if (set._w !== targetW) {
        gsap.set(set.panel, { width: targetW, height: targetH });
        set._w = targetW;
      }
      const d0 = i - k;
      ...
      let slot = morphing || forceMorph ? lerpSlot(from, to, t) : from;

      if (one) {
        if (phoneSettled) {
          slot = layouts.full[slotName(i, k)];
        } else {
          const parkFrom = layouts.full[slotName(i, k)];
          const parkTo = layouts.full[slotName(i, k2)];
          const parkSlot = lerpSlot(parkFrom, parkTo, t);
          const triFrom = (k >= maxP ? layouts.full : layouts.peek)[
            slotName(i, k)
          ];
          const triTo = (k2 >= maxP ? layouts.full : layouts.peek)[
            slotName(i, k2)
          ];
          const triSlot = lerpSlot(triFrom, triTo, t);
          const pull = Math.max(gestS, 0.22);
          slot = lerpSlot(parkSlot, triSlot, pull);
          // Pin whatever is currently "main" to full-bleed — never 78%
          const role = slotName(i, Math.round(clamped));
          if (role === "main" || (i === k && t < 0.5) || (i === k2 && t >= 0.5)) {
            const fullMain = layouts.full.main;
            slot = {
              ...slot,
              x: fullMain.x,
              y: fullMain.y,
    ...
    if (k === k2 && !snapTween && !phoneDragging) parkClasses(k);
  ...
  function snapTo(target, duration = CFG.snapDur, velocity = 0) {
    ...
    snapTween = gsap.to(state, {
      ...
      onComplete: () => {
        viewP = target;
        parkClasses(target);
        // Clear snap BEFORE park render — phoneSettled keys off !snapTween
        snapTween = null;
        setMorphing(false);
        render(viewP); // re-apply full vw park layout
        commitHash();

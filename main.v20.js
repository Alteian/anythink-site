(() => {
  ...
  function render(p) {
    ...
    const one = document.documentElement.classList.contains("deck-one");
    const phoneDragging = one && !!(touch && touch.locked);
    // morphing (k!==k2) is true on non-last integer pages — settle on atRest
    const atRest = Math.abs(clamped - Math.round(clamped)) < 1e-4;
    const phoneSettled = one && !phoneDragging && !snapTween && atRest;
    ...
    if (one) {
      // Width never changes on phone — transforms/opacity only
      targetW = layouts.full.mainW;
      targetH = layouts.full.mainH;
    } else if (morphing || forceMorph) {
      targetW = lerp(layFrom.mainW, layTo.mainW, t);
    } else {
      targetW = layFrom.mainW;
    }

    const away = Math.min(Math.abs(clamped - Math.round(clamped)), 0.5);
    const gest = one ? Math.min(1, away / 0.1) : 1;
    const gestS = gest * gest * (3 - 2 * gest);

    setters.forEach((set, i) => {
      if (one || set._w !== targetW) {
      ...
      if ((d0 < -1 || d0 > 3) && (d1 < -1 || d1 > 3)) {
        const hideX = (one ? layouts.full.mainW : L.mainW) + 140;
        set.x(hideX);
        set.o(0);
        set.s(0.4);
        set.r(0);
      ...
      if (one) {
        if (phoneSettled) {
          ...
          if (i === active) {
            slot = layouts.full.main;
          } else {
            // Must fully leave — stuck peeks were next/further with leftover transforms
            slot = {
              x: layouts.full.mainW + 140,
              y: layouts.full.main.y,
              s: 0.4,
              o: 0,
              r: 0,
              z: 1,
          ...
          // Fancy triangle unwrap: page morph in peek layout
          const fromL = k >= maxP ? layouts.full : layouts.peek;
          const toL = k2 >= maxP ? layouts.full : layouts.peek;
          const triSlot = lerpSlot(
            fromL[slotName(i, k)],
            toL[slotName(i, k2)],
            t
          );
          // Softly reveal peeks from off-screen at gesture start
          const parkSlot = lerpSlot(
            layouts.full[slotName(i, k)],
            layouts.full[slotName(i, k2)],
            t
          );
          const pull = Math.min(1, Math.max(gestS, 0.35));
          slot = lerpSlot(parkSlot, triSlot, pull);
          // Main role keeps s=1 while exiting (slide/fade only)
          if (i === k) {
            slot = { ...slot, s: 1 };
          }
        }
        ...
        slot = morphing || forceMorph ? lerpSlot(from, to, t) : from;
      }

      applySlot(set, slot, stackZ(i, clamped));

      const pe = one
        ? phoneSettled && slot.o > 0.5
          ? "auto"
          : slot.o > 0.55 && i === Math.round(clamped)
            ? "auto"
            : "none"
        : slot.o > 0.35
          ? "auto"
    ...
    if (phoneSettled || (!one && k === k2 && !snapTween)) {
      parkClasses(one ? Math.round(clamped) : k);
    }
  ...
  /** Phone only: one visible full card; every other scene gone */
  function forcePhonePark(activeI) {
    if (!document.documentElement.classList.contains("deck-one")) return;
  ...
  function setMorphing(on) {
    ...
    // Let peeks show again while a finger swipe is live
    if (on && document.documentElement.classList.contains("deck-one")) {
      panels.forEach((panel) => {
  ...
  function snapTo(target, duration = CFG.snapDur, velocity = 0) {
    ...
    if (Math.abs(target - viewP) < 1e-4) {
      ...
      snapTween = null;
      setMorphing(false);
      if (document.documentElement.classList.contains("deck-one")) {
        forcePhonePark(target);
      } else {
        parkClasses(target);
        render(viewP);
      }
  ...
  window.addEventListener(
    ...
    (e) => {
      ...
      const oneWheel = document.documentElement.classList.contains("deck-one");
      if (deckBusy()) {
  ...
  function endTouch() {
    ...
    touch = null;
    if (!wasLocked) {
      // Tap / cancelled mid-snap without a drag — still hard-park on phone
      // (otherwise peeks from an interrupted snap stay stuck).
      if (one) forcePhonePark(Math.round(viewP));
      else {
        setMorphing(false);
        render(viewP);
      }

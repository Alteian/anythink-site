(() => {
  ...
  function layout() {
    ...
    if (oneCard) {
      ...
      // Parked: full-bleed main; peeks keep triangle xy so they can fade in
      // without flying in from off-screen when a swipe starts.
      const park = {
        mainW: stageW,
        mainH: stageH,
        main: { x: padX, y: padY, s: 1, o: 1, r: 0, z: 5 },
        next: { ...tri.next, o: 0 },
        further: { ...tri.further, o: 0 },
        enter: { ...tri.enter, o: 0 },
        exit: { ...tri.exit, o: 0 },
      };
  ...
  function render(p) {
    ...
    setters.forEach((set, i) => {
      ...
      if (one && (morphing || forceMorph)) {
        ...
        // First fifth: park → triangle; last fifth: triangle → park
        const rise = Math.min(1, t / 0.18);
        const fall = Math.min(1, (1 - t) / 0.18);
  ...
  function snapTo(target, duration = CFG.snapDur) {
    ...
    // Same triangle unwrap language on phone; slightly snappier settle
    const ease = fromHero ? "power3.out" : one ? "power3.out" : CFG.snapEase;
    const state = { p: viewP };
    setMorphing(true);

(() => {
  ...
  function layout() {
    ...
    // Phone (clean): every card is full-vw size. Main never shrinks.
    // Mid-swipe: main slides/fades left; next grows from right peek; further
    // tucks top-right (triangle). Scale is transform-only — width stays vw.
    if (oneCard) {
      const parkBottom = 28;
      const parkH = Math.max(200, vh - headerBottom - parkBottom);
      const mainW = vw;
      const mainH = parkH;
      const peekLeft = Math.round(vw * 0.56);
      const nextS = 0.46;
      const furtherS = 0.4;
      const nextTop = headerBottom + Math.round(parkH * 0.38);
      const furtherTop = headerBottom + 10;
      ...
      // Triangle peeks sit over the right edge of the full-bleed main
      const nextPeek = {
        x: peekLeft,
        y: nextTop,
        s: nextS,
      ...
      const tri = {
        mainW,
        mainH,
        // Mid-swipe "main" starts full; page morph sends it toward exit
        main: {
          x: 0,
          y: headerBottom,
          s: 1,
        ...
        // Slide left + fade — keep s:1 so content never squashes
        exit: {
          x: -Math.round(vw * 0.42),
          y: headerBottom,
          s: 1,
          o: 0,
          r: -1.5,
      ...
      const park = {
        ...
        mainH,
        main: { ...mainSlot },
        next: { ...nextPeek, x: vw + 36, o: 0 },
        further: { ...furtherPeek, x: vw + 64, o: 0 },
        enter: { ...tri.enter, o: 0 },
        exit: { ...tri.exit, o: 0 },
      };

      return { peek: tri, full: park };
    }
  ...
  function sizeCards() {
    ...
    // Phone parked metrics (full vw) — never seed cards at triangle ~78% width
    const w = one ? layouts.full.mainW : L.mainW;
    const h = one ? layouts.full.mainH : L.mainH;
  ...
  const CFG = {
    ...
    wheelCooldownMs: 50,
    touchCommit: 0.12, // desktop; phone uses touchCommitPhone
    touchCommitPhone: 0.035, // short swipe/scroll commits a page
    touchDragGainPhone: 1.55, // finger travels less for same page progress
    touchFlickPhone: 0.35,
    touchProjectPhone: 0.22,
  };
  ...
  // ---------------------------------------------------------------------------
  // Phone deck state machine (idle | dragging | snapping)
  // Desktop path below is intentionally separate and behavior-frozen.
  // ---------------------------------------------------------------------------
  /** @type {'idle' | 'dragging' | 'snapping'} */
  let phonePhase = 'idle';

  function isPhone() {
    return document.documentElement.classList.contains('deck-one');
  }

  function setPhonePhase(phase) {
    phonePhase = phase;
    if (isPhone()) {
      document.documentElement.classList.toggle('deck-morphing', phase !== 'idle');
      // Allow peeks to paint; never clear opacity (CSS .deck-card { opacity:0 } would win).
      if (phase !== 'idle') {
        panels.forEach((panel) => {
  ...
  /** Phone only: one full-vw card; every other card fully gone. Clears leftovers. */
  function hardParkPhone(activeI) {
    if (!isPhone()) return;
    cancelSnap();
    activeI = Math.max(0, Math.min(maxP, Math.round(activeI)));
    viewP = activeI;
    phonePhase = 'idle';
    document.documentElement.classList.remove('deck-morphing');
    const park = layouts.full;
    const hideX = (park.mainW || window.innerWidth) + 140;
    const y = park.main.y;
    parkClasses(activeI, true);
    setters.forEach((set, i) => {
      gsap.killTweensOf(set.panel);
      gsap.set(set.panel, {
        width: park.mainW,
        height: park.mainH,
        force3D: true,
      ...
      set._w = park.mainW;
      if (i === activeI) {
        // Drive opacity only via GSAP — never style.opacity="" (CSS .deck-card { opacity:0 } wins).
        applySlot(set, { ...park.main, o: 1 }, 50);
        set.panel.style.pointerEvents = 'auto';
        set.panel.style.visibility = 'visible';
        set._pe = 'auto';
        set._lit = true;
      } else {
        applySlot(set, { x: hideX, y, s: 0.4, o: 0, r: 0 }, 1);
        set.panel.style.pointerEvents = 'none';
        set.panel.style.visibility = 'hidden';
        set._pe = 'none';
        set._lit = false;
      }
    });
  ...
  function renderPhone(p) {
    ...
    const targetW = layouts.full.mainW;
    const targetH = layouts.full.mainH;

    // Parked: only the committed page; no peeks
    if (phonePhase === 'idle') {
      const active = Math.round(clamped);
      const hideX = targetW + 140;
      setters.forEach((set, i) => {
        if (set._w !== targetW) {
        ...
        if (i === active) {
          applySlot(set, { ...layouts.full.main, o: 1 }, 50);
          set.panel.style.visibility = 'visible';
          if (set._pe !== 'auto') {
            set.panel.style.pointerEvents = 'auto';
            set._pe = 'auto';
          ...
          applySlot(
            set,
            { x: hideX, y: layouts.full.main.y, s: 0.4, o: 0, r: 0 },
            1
          );
          set.panel.style.visibility = 'hidden';
          if (set._pe !== 'none') {
    ...
    // dragging | snapping: triangle peeks track progress (unwrap until complete)
    const fromL = k >= maxP ? layouts.full : layouts.peek;
    const toL = k2 >= maxP ? layouts.full : layouts.peek;
    ...
    setters.forEach((set, i) => {
      ...
      let slot = lerpSlot(fromL[slotName(i, k)], toL[slotName(i, k2)], t);
      // Main role keeps s=1 while exiting (slide/fade only — no width morph)
      if (i === k) slot = { ...slot, s: 1 };

      applySlot(set, slot, stackZ(i, clamped));

      const pe =
        slot.o > 0.55 && i === Math.round(clamped) ? 'auto' : 'none';
      if (set._pe !== pe) {
  ...
  /** Desktop triangle layout — behavior frozen (50ms lock, peeks always). */
  function renderDesktop(p) {
    ...
    let targetW;
    let targetH = layouts.peek.mainH;
  ...
  function parkClasses(mainI, force = false) {
    mainI = Math.max(0, Math.min(maxP, Math.round(mainI)));
    if (!force && mainI === lastClassI) return;
    lastClassI = mainI;
  ...
  function setMorphing(on) {
    // Desktop-only helper; phone drives deck-morphing via setPhonePhase
    if (isPhone()) return;
    document.documentElement.classList.toggle('deck-morphing', on);
  }

  function snapTo(target, duration = CFG.snapDur, velocity = 0) {
    cancelSnap();
    pendingDir = 0;
    ...
    let ease;
    if (one) {
      // iOS-sheet-ish: duration from remaining distance + finger velocity
      const v = Math.max(0.45, Math.abs(velocity));
      dur = Math.max(0.4, Math.min(0.88, (dist / v) * 0.7 + 0.22));
      if (fromHero) dur = Math.min(0.95, dur + 0.06);
      ease = 'expo.out';
      setPhonePhase('snapping');
    } else {
      dur = Math.max(
        fromHero ? 0.42 : 0.3,
        Math.min(duration, (fromHero ? 0.34 : 0.22) + dist * 0.28)
      );
      ease = fromHero ? 'power3.out' : CFG.snapEase;
      setMorphing(true);
    }
    const state = { p: viewP };
    // Lock for the whole unwrap + post-snap break (no chained second scene)
    // Phone: interruptible (0 extra lock). Desktop: keep CFG.wheelCooldownMs (50).
    const lockExtra = one ? 0 : CFG.wheelCooldownMs;
    armDeckLock(Math.round(dur * 1000) + lockExtra);
  ...
  // Firm page snap: one gesture = one scene (no mid-unwrap float)
  window.addEventListener(
    ...
    (e) => {
      ...
      const oneWheel = isPhone();
      if (deckBusy()) {
        if (oneWheel && snapTween) {
          cancelSnap();
        } else {
          return;
        }
  ...
  window.addEventListener(
    ...
    (e) => {
      ...
      // Phone: no scroll brake — kill in-flight snap and continue from here.
      // Desktop: keep the post-snap lock.
      if (deckBusy()) {
        if (isPhone() && snapTween) {
          cancelSnap();
          setPhonePhase('dragging');
        } else {
          touch = null;
          return;
        }
      ...
      touch = {
        ...
        locked: false,
        axis: null,
        samples: [{ t: performance.now(), p: viewP }],
  ...
  window.addEventListener(
    ...
    (e) => {
      ...
      if (!touch.locked) {
        ...
        touch.axis =
          isPhone() && Math.abs(dx) > Math.abs(dy) ? 'x' : 'y';
        if (isPhone()) setPhonePhase('dragging');
        else setMorphing(true);
      }

      e.preventDefault();
      touch.x = t.clientX;
      touch.y = t.clientY;
      if (!isPhone() && !touch._morphOn) {
        touch._morphOn = true;
        setMorphing(true);
      }
      const drag =
      ...
      // Phone: gain so a short swipe fills page progress faster
      const one = isPhone();
      const gained = one ? drag * CFG.touchDragGainPhone : drag;
      const raw = touch.p0 + gained;
      viewP = one
        ? Math.max(-0.18, Math.min(maxP + 0.18, raw))
        : Math.max(-0.15, Math.min(maxP + 0.15, raw));
      const now = performance.now();
      touch.samples.push({ t: now, p: viewP });
      while (touch.samples.length > 8) touch.samples.shift();
      render(viewP);
    },
    { passive: false }
  );

  function touchVelocity() {
    if (!touch || !touch.samples || touch.samples.length < 2) return 0;
    const now = performance.now();
    // Last ~90ms of motion (finger velocity, not whole gesture average)
    const windowMs = 90;
    let i = touch.samples.length - 1;
    const last = touch.samples[i];
    while (i > 0 && last.t - touch.samples[i - 1].t < windowMs) i -= 1;
    const first = touch.samples[i];
    const dt = Math.max(1 / 120, (last.t - first.t) / 1000);
  ...
  function endTouch() {
    if (!touch) return;
    const one = isPhone();
    const wasLocked = touch.locked;
    const p0 = touch.p0;
    const vel = wasLocked ? touchVelocity() : 0;
    const delta = viewP - p0;
    const commit = one ? CFG.touchCommitPhone : CFG.touchCommit;
    const flick = one ? CFG.touchFlickPhone : 0.7;
    const project = one ? CFG.touchProjectPhone : 0.4;
    let target = Math.round(viewP);
    // Prefer nearest page from current + velocity projection (no hard cut)
    if (wasLocked) {
      const projected = viewP + vel * (one ? 0.22 : 0.18);
      if (delta > commit || vel > flick || projected > p0 + project)
        target = Math.floor(p0) + 1;
      else if (delta < -commit || vel < -flick || projected < p0 - project)
        target = Math.ceil(p0) - 1;
      else target = Math.round(viewP);
    }
    target = Math.max(0, Math.min(maxP, target));
    touch = null;
    if (!wasLocked) {
      // Tap / cancelled mid-snap without a drag — always hard-park on phone
      if (one) hardParkPhone(Math.round(viewP));
      else {
        setMorphing(false);
        render(viewP);
      }
  ...
  function relayoutDeck() {
    ...
    if (isPhone() && phonePhase === 'idle') hardParkPhone(viewP);
    else render(viewP);
  }
  ...
  function bootDeck() {
    ...
    try {
      ...
      L = layouts.peek;
      sizeCards();
      if (isPhone()) hardParkPhone(start);
      else render(viewP);
      commitHash();
      panels.forEach((p, i) => {

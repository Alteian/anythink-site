(() => {
  ...
  window.addEventListener(
    ...
    (e) => {
      ...
      // Phone: slightly rubber-banded follow so peeks track the finger
      const one = isPhone();
      const raw = touch.p0 + drag;
      viewP = one
  ...
  function endTouch() {
    ...
    const flick = one ? 0.55 : 0.7;
    let target = Math.round(viewP);
    // Prefer nearest page from current + velocity projection (no hard cut)
    if (wasLocked) {
      const projected = viewP + vel * 0.18;
      if (delta > commit || vel > flick || projected > p0 + 0.4)
        target = Math.floor(p0) + 1;
      else if (delta < -commit || vel < -flick || projected < p0 - 0.4)

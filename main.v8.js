(() => {
  ...
  function render(p) {
    ...
    // Phone at rest: full-bleed park (peeks hidden). Mid-swipe: triangle peeks.
    const phoneParked = one && !snapTween && !morphing;

    const layFrom = phoneParked ? layouts.full : layForPage(k);
    const layTo = phoneParked ? layouts.full : layForPage(k2);
    let targetW = layFrom.mainW;
    if (morphing || forceMorph) {
      targetW = lerp(layFrom.mainW, layTo.mainW, t);
      if (one) {
        const triW = layouts.peek.mainW;
        const parkW = layouts.full.mainW;
        const rise = Math.min(1, t / 0.32);
        const fall = Math.min(1, (1 - t) / 0.28);
        const fromW = lerp(parkW, triW, rise);
        const toW = lerp(parkW, k2 >= maxP ? parkW : triW, fall);
        targetW = lerp(fromW, toW, t);
      }
    } else {
    ...
    setters.forEach((set, i) => {
      ...
      let from = layFrom[slotName(i, k)];
      let to = layTo[slotName(i, k2)];
      // Phone: fade peeks in/out around the triangle unwrap (hidden when parked)
      if (one && (morphing || forceMorph)) {
        ...
        // Side → triangle tuck, then drift into park (phone only)
        const rise = Math.min(1, t / 0.32);
        const fall = Math.min(1, (1 - t) / 0.28);
        from = lerpSlot(parkFrom, triFrom, rise);
        to = lerpSlot(parkTo, triTo, fall);
      }
  ...
  function snapTo(target, duration = CFG.snapDur) {
    ...
    const one = document.documentElement.classList.contains("deck-one");
    // Phone: slower side→triangle drift. Desktop timings untouched.
    const dur = one
      ? Math.max(
          fromHero ? 0.78 : 0.64,
          Math.min(1.05, (fromHero ? 0.62 : 0.52) + dist * 0.36)
        )
      : Math.max(
          fromHero ? 0.42 : 0.3,
